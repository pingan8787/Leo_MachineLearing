...
在图的构建阶段，就是各种op的拼接组合，op之间流通的tensor是由最初的一个op产生的，它被称为源op，没有输入tensor，只有输出tensor。
比如说常量(Constant)就是一个源op：
'''
...
'''
我们这样就创建了一个最简单的图，图的输出端口为product，这个图包含了三个op，两个源constant()op,一个matmul()op
如果要想真正的执行得到乘法之后的结果，需要运行会话，然后获取product的值。
Session对象在使用完之后需要手动关闭close()，也可以使用‘with’让程序自动的执行完之后关闭。
'''
